...
class LiquidationHunter:
    ...
    def _calculate_position_size(self, exchange: ExchangeOpening, confidence: float) -> float:
        """Calculate recommended position size based on opportunity"""
        base_size = self.max_position_size * confidence
        
        # Adjust for exchange importance
        importance_multiplier = exchange.importance / 5.0
        
        # Adjust for liquidation risk
        risk_multiplier = exchange.liquidation_risk
        
        recommended_size = base_size * importance_multiplier * risk_multiplier
        
        return min(self.max_position_size, max(0.01, recommended_size))  # Min 1%, Max 10%
